...
class SpotifyAlbum(SpotifyObject):
    def __init__(self, name=None, id=None, release_date=None, images=None, info_dic={}):
        ...
        self.release_date = (
            info_dic.get("release_date") if not release_date else release_date
        )
...
class SpotifyAPI(object):
    ...
    resource_lookup_root = (
        "https://api.spotify.com/{version}/{resource_type}/{lookup_id}"
    )
    ...
    def _format_search_params(self, extra_search_params):
        ...
        if not isinstance(extra_search_params, dict):
            raise Exception(
                "Additional parameters should be specified in a dictionary object"
            )

        return "&".join(
            [f"{key}={value}" for key, value in extra_search_params.items()]
        )
    ...
    def get_resource(
        ...
        endpoint = self.resource_lookup_root.format(
            version=version, resource_type=resource_type, lookup_id=lookup_id
        )

        if extra_search is not None:
            endpoint += (
                f"/{extra_search}?{self._format_search_params(extra_search_params)}"
            )
    ...
    def playlist_tracks(self, playlist_id):
        ...
        while _next:
            ...
            tracks_dic = SpotifyAPI(self.client_id, self.secret_id).get_resource(
                lookup_id=playlist_id,
